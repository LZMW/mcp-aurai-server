...
> **重要**：使用 `--scope user` 确保在所有项目中都可用，避免每次切换目录都要重新安装。
...
| 删除配置 | `claude mcp remove aurai-advisor -s user` |
| 更新配置 | 删除后重新添加 |
...
claude mcp add --scope user --transport stdio aurai-advisor \
  ...
  --env AURAI_TEMPERATURE="0.7" \
  --env AURAI_MAX_TOKENS="4096" \
  --env AURAI_HISTORY_PATH="D:\\mcp-aurai-server\\mcp_conversation_history.json" \
